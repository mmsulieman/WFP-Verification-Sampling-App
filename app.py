...
#  - Reallocate village shortfalls within kebele
#  - NumPy dtype fix (pandas-native Group assignment)
#  - kebele summary concat hotfix
#  - NEW: Auto-add extra village(s) option if selected villages cannot meet per-group target
...
with st.sidebar:
    ...
    auto_add_extra = st.checkbox('Auto‑add extra village(s) if selected villages cannot meet kebele target', value=True)
    max_extra_villages = st.number_input('Max extra villages to add (per kebele)', min_value=0, max_value=50, value=4, step=1)
    auto_add_strategy = st.selectbox('Auto‑add selection strategy for remaining villages', ['PPS by total HHs', 'Largest capacity (Eligible+Non-eligible)'])
...
if st.button('Run PPS + Systematic Sampling'):
    ...
    for kebele, df_k in work.groupby(col_kebele):
        ...
        # ---- NEW: Auto-add extra villages if selected set cannot meet per-group targets ----
        auto_added = []
        if auto_add_extra:
            # Compute current capacities for selected villages per group
            def group_capacity(villages, grp):
                return int(((df_k[col_village].isin(villages)) & (df_k['Group']==grp)).sum())
            cap_el = group_capacity(selected_villages, 'Eligible')
            cap_ne = group_capacity(selected_villages, 'Non-eligible')

            remaining = [v for v in df_k[col_village].unique().tolist() if v not in selected_villages]
            added = 0
            while (cap_el < per_group or cap_ne < per_group) and remaining and added < int(max_extra_villages):
                # Determine weights for remaining villages
                if auto_add_strategy == 'PPS by total HHs':
                    # total HHs per remaining village (any group)
                    counts = [int((df_k[col_village]==v).sum()) for v in remaining]
                else:
                    # Largest capacity by Eligible+Non-eligible
                    counts = [int(((df_k[col_village]==v) & (df_k['Group']=='Eligible')).sum() + ((df_k[col_village]==v) & (df_k['Group']=='Non-eligible')).sum()) for v in remaining]
                total = sum(counts)
                if total == 0:
                    break
                probs = np.array(counts, dtype=float) / float(total)
                idx = int(rng.choice(len(remaining), size=1, replace=False, p=probs))
                new_v = remaining.pop(idx)
                selected_villages.append(new_v)
                auto_added.append(new_v)
                # Update capacities
                cap_el += int(((df_k[col_village]==new_v) & (df_k['Group']=='Eligible')).sum())
                cap_ne += int(((df_k[col_village]==new_v) & (df_k['Group']=='Non-eligible')).sum())
                added += 1
        ...
        # Log per-village plan after rebalance (mark auto-added)
        auto_added_set = set(auto_added)
        for i, v in enumerate(selected_villages):
            ...
            village_selection_rows.append({
                ...
                'Auto_Added': v in auto_added_set
            })
        ...
        tmp = pd.concat(hh_rows, ignore_index=True) if hh_rows else pd.DataFrame()
        if not tmp.empty:
        ...
        kebele_summary_rows.append({
            ...
            'Village_Selection_Mode': rule_used,
            'Auto_Added_Count': len(auto_added_set),
    ...
    with pd.ExcelWriter(out_buf, engine='openpyxl') as xw:
        ...
        pd.DataFrame({
            'Parameter': ['Seed','Mode','Per_Group_Sample','UniqueOnly','Synthetic_HH_ID','Reserve_Per_GroupPerVillage','Auto_Add_Extra_Villages','Max_Extra_Villages','Auto_Add_Strategy'],
            'Value': [seed, mode, per_group_sample, True, synthetic_id, reserve_count, auto_add_extra, max_extra_villages, auto_add_strategy]
